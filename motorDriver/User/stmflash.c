...
//    }
	...
	if(FlashStatus==HAL_OK)
	{
		...
		{
            if(HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,addrx,(uint32_t)*pBuffer)!=HAL_OK)//写入数据
			{ 
				break;	//写入异常
			}
			addrx+=4;	//每个FLASH地址  存储 0xFF 8bit 数据  32*8=256
			pBuffer+=1;	// 8*32 = 256  数据类型为u32  即指针向后移8  移过256bit
		} 
	}
	HAL_FLASH_Lock();           //上锁
} 
